...
namespace NaLP___Server
{
    public class Encryption
    {
        ...
        private static SafeQuickLZ uQLZ = new SafeQuickLZ();
        ...
        public byte[] AES_Encrypt(byte[] bytesToBeEncrypted)
        {
            ...
            bOut = bOut.Concat(bIV).ToArray();

            var key = new PasswordDeriveBytes(bKey, bOut);
            ...
            {
                bOut = bOut.Concat(iCT.TransformFinalBlock(bytesToBeEncrypted, 0, bytesToBeEncrypted.Length)).ToArray();
            }

            bOut = bOut.Concat(new HMACSHA256(bHKey).ComputeHash(bOut.Skip(16).Take(bOut.Length - 16).ToArray())).ToArray();

            return Compress(bOut);
        }
        ...
        public byte[] AES_Decrypt(byte[] bytes)
        {
            ...
            var key = new PasswordDeriveBytes(bKey, bIn.Take(16).ToArray());
            ...
            byte[] bytesToBeDecrypted = bIn.Skip(16).Take(bIn.Length - 32 - 16).ToArray();

            if (!new HMACSHA256(bHKey).ComputeHash(bytesToBeDecrypted).SequenceEqual(bIn.Skip(bIn.Length - 32).Take(32).ToArray()))
        ...
        {
            return uQLZ.decompress(input, 0);
        }
