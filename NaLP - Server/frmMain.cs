    ...
    private void itemBan_Click(object sender, EventArgs e)
    {
      if (dgvConnections.SelectedCells?.Count > 0)
      {
        ...
        if (cEP != null)
        {
          ...
          if (server.Clients[cEP].sClass.isLoggedIn)
          {
            File.AppendAllText("BannedHWIDs.txt", Encoding.UTF8.GetString(server.Clients[cEP].sClass.thisClient.bHWID) + Environment.NewLine);
            File.Delete(string.Format("Clients\\{0}.xml", server.Clients[cEP].sClass.thisClient.sUsername));
          }
