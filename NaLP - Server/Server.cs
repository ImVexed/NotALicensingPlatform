...
namespace NaLP___Server
{
  ...
  public class Server
  {
    ...
    private Action<string, Color> aLog;
    ...
    public void Start(Action<string, Color> aLog)
    {
      ...
      Log("Server started!", Color.Green);
      ...
      foreach (MethodInfo mI in typeof(SharedClass).GetMethods())
      {
        ...
        if (oAttr.Length > 0)
        {
          ...
          Log($"Identifier {thisAttr.Identifier} MethodInfo link created...", Color.Green);
    ...
    //Our method for accepting clients
    {
      ...
      catch { Log("Server stopping!", Color.Green);  return; } // This will happen when we shutdown the server

      Log($"Client connected from IP: {client.RemoteEndPoint}", Color.Green, true);
      ...
      var sC = new sClient
      {
        ...
        eCls = null,
        bSize = new byte[4],
        sClass = new SharedClass(client.RemoteEndPoint)
      };
    ...
    // Handshake + Encryption is handled outside of this callback, so any message that makes it here is expected to be a method call/move.
    {
      ...
      try
      {
        ...
        {
          Log($"Client IP: {sEP} has disconnected...", Color.Orange, true);
          sC.cSocket.Close();
        ...
        Log($"Receiving {cBuffer.Length} bytes...", Color.Cyan);
        ...
        {
          Log($"Client IP: {sEP} caused an exception invoking {oMsg[1]}", Color.Orange);
          oRet[0] = Headers.HEADER_ERROR;
        }

        Log($"Client IP: {sEP} called Remote Identifier: {oMsg[1]}", Color.Cyan);
      ...
      {
        Log($"Client IP: {sEP} has caused an exception...", Color.Orange, true);
    ...
    private void BlockingSend(sClient sC, params object[] param)
    {
      ...
      Log($"Sending {bSend.Length} bytes...", Color.Cyan);
    ...
    private object[] BlockingReceive(sClient sC)
    {
      ...
      Log($"Receiving {sBuf.Length} bytes...", Color.Cyan);
